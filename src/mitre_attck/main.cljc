(ns mitre_attck.main
  "Kotodama WASM entrypoint for the MITRE ATT&CK clean-room actor (L5) — Clojure port.
  ...
  facts (`mitre_attck.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "mitre_attck")
(def tier "L5")
...
(def entity-specs
  [{:entity "Technique" :plural "techniques" :id-prefix "mitreatt_tec"
    :fields [:techniqueId :name :description :isSubtechnique :created :modified] :required [:name :description]
    :coerce {:isSubtechnique :bool}   :refs {}}
   {:entity "Tactic" :plural "tactics" :id-prefix "mitreatt_tac"
    :fields [:tacticId :shortName :name :description :created] :required [:shortName :name]
    :coerce {}                 :refs {}}
   {:entity "Software" :plural "softwares" :id-prefix "mitreatt_sof"
    :fields [:softwareId :name :type :description :isFamily :created] :required [:name :type]
    :coerce {:isFamily :bool}    :refs {}}
   {:entity "Group" :plural "groups" :id-prefix "mitreatt_gro"
    :fields [:groupId :name :description :created] :required [:name :description]
    :coerce {}                 :refs {}}
   {:entity "Mitigation" :plural "mitigations" :id-prefix "mitreatt_mit"
    :fields [:mitigationId :name :description :deprecated :created] :required [:name :description]
    :coerce {:deprecated :bool}   :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"mitre_attck.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "mitre_attck-compat" :tier tier :entities entities} 200])
...
(defn start! [] :mitre_attck-compat/ready)
